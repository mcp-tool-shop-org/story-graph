...
const idempotencyCache = new Map<
  string,
  { bodyHash: string; response: { storyId: string; versionId: string; createdAt: string }; expiresAt: number; requestId: string }
>();
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (typeof body.content !== 'string') {
      return errorResponse('invalid_content', 'content must be a YAML string', 400, undefined, requestId);
    }
    ...
    if (idempotencyKey) {
      const persistedHit =
        activeStoryStore instanceof SQLiteStoryStore ? activeStoryStore.getIdempotency(idempotencyKey) : undefined;
      if (persistedHit) {
        if (persistedHit.requestHash !== bodyHash) {
          return finish(
            errorResponse('idempotency_conflict', 'Idempotency-Key already used with different payload', 409, undefined, requestId),
            409,
            { idempotencyHit: true, idempotencyAgeMs: Date.now() - new Date(persistedHit.createdAt).getTime() }
          );
        }
        return finish(
          jsonResponse(
            { storyId: persistedHit.storyId, versionId: persistedHit.versionId, createdAt: persistedHit.createdAt },
      ...
      if (cached && notExpired) {
        return finish(jsonResponse(cached.response, { status: 200, requestId: cached.requestId }), 200, {
          idempotencyHit: true,
    ...
    const expected = typeof body.expectedVersionId === 'string' ? body.expectedVersionId : request.headers.get('if-match') ?? undefined;
    ...
    if (err.name === 'ConflictError') {
      return finish(
        errorResponse('version_conflict', 'Version conflict', 409, { latestVersionId: activeStoryStore.get(params.id)?.latestVersionId }, requestId),
        409
      );
    }
    if (err.name === 'DatabaseUnavailableError') {
      return finish(errorResponse('store_unavailable', 'Database is busy, please retry', 503, undefined, requestId), 503);
    }
